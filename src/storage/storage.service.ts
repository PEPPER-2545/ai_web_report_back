...
export class StorageService implements OnModuleInit {
    ...
    // --- ฟังก์ชันใหม่ (สำหรับ MIOC Web / mioc-report Bucket) ---

    async uploadMiocFile(file: Express.Multer.File, fileName: string, folder: string): Promise<string> {
        const bucket = 'mioc-report';
        const fullPath = `${folder}/${fileName}`;
        
        await this.minioClient.putObject(bucket, fullPath, file.buffer, file.size, {
            'Content-Type': file.mimetype,
        });

        // ส่งกลับเป็น Full URL
        const endpoint = this.configService.get('MINIO_ENDPOINT');
        const protocol = this.configService.get('MINIO_USE_SSL') === 'true' ? 'https' : 'http';
        return `${protocol}://${endpoint}/${bucket}/${fullPath}`;
    }

    async deleteMiocFile(fileUrl: string): Promise<void> {
        const bucket = 'mioc-report';
        try {
            const endpoint = this.configService.get('MINIO_ENDPOINT');
            const baseUrl = `${endpoint}/${bucket}/`;
            if (fileUrl && fileUrl.includes(baseUrl)) {
                const urlPath = fileUrl.split(baseUrl)[1];
                if (urlPath) {
                    await this.minioClient.removeObject(bucket, urlPath);
                }
            }
        } catch (error) {
            console.error('Error deleting MIOC file:', error);
        }
    }
}